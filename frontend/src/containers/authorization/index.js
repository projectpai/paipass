...
class Authorization extends React.Component {
  ...
  componentDidMount() {
    ...
    const createPermsStatus = ({ scopes }) => {
      ...
      // delimited by periods with a corresponding value corresponding to
    ...
    const transformResponse = R.cond([
      ...
      [
        // make sure name is not empty before we set the stat
        R.propSatisfies(
          R.compose(
            R.not,
            R.isEmpty,
    ...
    this.props.oauthService
      .getAuthorizeDetails({ clientID, scopeList: scopeList(scopes) })
  ...
  render() {
    ...
    const paiforum_app_name = 'PAI Forum';
    if (details.name === paiforum_app_name){
      app_name = getString('oauth-permission-application-name-hack');
      description = getString('oauth-permission-description-hack');
    ...
    return (
      <div className="container-fluid authorization-rt">
        ...
        <div className="row justify-content-center align-items-center flex-column text-center">
          <form
            action={`${URL_BASE}/${
              process.env.REACT_APP_API_OAUTH_AUTHORIZE_EP
            }${search}`}
            method="post"
          >
            <input type="hidden" name="csrfmiddlewaretoken" value={Cookies.get('csrftoken')} />
            <h2> <Localized id="oauth-permission-application-title" value={"Offer from { $application }"} vars={{application:app_name}}/> </h2>
            <div className="mb-1">
              <p>{description}</p>
              <p>
                <Localized id="oauth-permission-prompt" value="is requesting access to following information from your profile:" vars={{application:app_name}}/>
              </p>
...
export default withRouter(connect(
  mapStateToProps,
  mapDispatchToProps,
)(withLocalization(Authorization)));
