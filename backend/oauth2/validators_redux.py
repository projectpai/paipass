...
from oauth2.models import PaipassGrant, AccessTokenScopes, PaipassApplication
...
# TODO see if it's possible to subclass Oauth2Validator...
...
class OAuth2Validator(RequestValidator):
    ...
    def save_authorization_code(self, client_id, code, request, *args, **kwargs):
        ...
        for scope in request.scopes:
            appr = AttributeApproval.objects.create(grant=g,
                                                    attribute=scope,
                                                    requesting_app=request.client,
                                                    owning_app=scope.application,
                                                    user=request.user)
            appr.save()
    ...
    def _create_access_token(self, expires, request, token, source_refresh_token=None):
        ...
        # if getattr(request, 'refresh_token', None) is not None:
        if source_refresh_token is not None:
            self.generate_ats_from_rt(source_refresh_token, access_token, request)
        else:
            self.generate_ats_from_at(request, access_token)
    ...
    def generate_ats_from_at(self, request, access_token):
        ...
        prev_at_scopes = self.get_prev_access_token_scopes(request.client, request.user)
        ...
        for appr in apprs:
            qs_ats = AccessTokenScopes.objects.all().filter(access_token=access_token, attribute=appr.attribute)
            if qs_ats.count() < 1:
                ats = AccessTokenScopes.objects.create(access_token=access_token,
    ...
    def get_prev_access_token_scopes(self, requesting_app, user):
        prev_access_tokens = AccessToken.objects.all().filter(application=requesting_app,
                                                              user=user)
        prev_access_token_scopes = []
        for prev_access_token in prev_access_tokens:
            atss = AccessTokenScopes.objects.all().filter(access_token=prev_access_token)
            for ats in atss:
                prev_access_token_scopes.append(ats)
        return prev_access_token_scopes

    def find_code(self, request):
        # this was where it was when I found it
        if request.decoded_body[1][0] == 'code':
            return request.decoded_body[1][1]
        # however, just in case it's somewhere else due to some possible differences ordering
        # that are unbeknownst to me, we will search for it
        for el in request.decoded_body:
            if el[0] == 'code' or el[0] == 'refresh_token':
                return el[1]
        raise ValueError(f'Code not found for {request} in its decoded body {request.decoded_body}')
    ...
    def get_original_scopes(self, refresh_token, request, *args, **kwargs):
        ...
        scopes = ''
        for ats in AccessTokenScopes.objects.all().filter(access_token=rt.access_token):
            perm = attributes.convert_from_enum_perms(ats.max_perms)
            namespace = ats.attribute.application.namespace
            scopes +=  perm +'.' + namespace + '.' + ats.attribute.name + ' '
        return scopes[:-1]

    def get_original_ats(self, refresh_token, request, *args, **kwargs):
        # Avoid second query for RefreshToken since this method is invoked *after*
        # validate_refresh_token.
        rt = request.refresh_token_instance
        if not rt.access_token_id:
        ...
        return AccessTokenScopes.objects.all().filter(access_token=rt.access_token)
