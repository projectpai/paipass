...
def _compare_salted_tokens(request_csrf_token, csrf_token):
    ...
    print('_unsalt_cipher_token(request_csrf_token)', _unsalt_cipher_token(request_csrf_token))
    print('_unsalt_cipher_token(csrf_token)', _unsalt_cipher_token(csrf_token))
...
class CsrfViewMiddleware(MiddlewareMixin):
    ...
    def _reject(self, request, reason):
        ...
        print('_reject ' + 'Forbidden (%s): %s', reason, request.path)
        return response
    ...
    def _get_token(self, request):
        if settings.CSRF_USE_SESSIONS:
            ...
            try:
                ret = request.session.get(CSRF_SESSION_KEY)
                print('request.session.get(CSRF_SESSION_KEY)' + str(ret))
                return ret
            except AttributeError:
                print("raise ImproperlyConfigured(('_CLASSES' if settings.MIDDLEWARE is None else '")
            ...
            print('else settings.CSRF_USE_SESSIONS')
            try:

                cookie_token = request.COOKIES[settings.CSRF_COOKIE_NAME]
                print('cookie_token = request.COOKIES[settings.CSRF_COOKIE_NAME]' + str(cookie_token))
            except KeyError:
            ...
            print('csrf_token = _sanitize_token(cookie_token)' + str(csrf_token))
            if csrf_token != cookie_token:
                print('if csrf_token != cookie_token: == true')
    ...
    def _set_token(self, request, response):
        if settings.CSRF_USE_SESSIONS:
            print('_set_token if settings.CSRF_USE_SESSIONS:')
            if request.session.get(CSRF_SESSION_KEY) != request.META['CSRF_COOKIE']:
                request.session[CSRF_SESSION_KEY] = request.META['CSRF_COOKIE']
        else:
            print('_set_token response.set_cookie()')
            print('settings.CSRF_COOKIE_NAME', settings.CSRF_COOKIE_NAME)
            print("request.META['CSRF_COOKIE']", request.META['CSRF_COOKIE'])
            print('max_age=settings.CSRF_COOKIE_AGE,', settings.CSRF_COOKIE_AGE)
            print('domain=settings.CSRF_COOKIE_DOMAIN', settings.CSRF_COOKIE_DOMAIN)
            print('path=settings.CSRF_COOKIE_PATH', settings.CSRF_COOKIE_PATH)
            print('secure=settings.CSRF_COOKIE_SECURE,', settings.CSRF_COOKIE_SECURE)
            print('httponly = settings.CSRF_COOKIE_HTTPONLY,', settings.CSRF_COOKIE_HTTPONLY)
            print('samesite=settings.CSRF_COOKIE_SAMESITE,', settings.CSRF_COOKIE_SAMESITE)
            response.set_cookie(
            ...
            print("patch_vary_headers(response, ('Cookie',))")
    ...
    def process_view(self, request, callback, callback_args, callback_kwargs):
        ...
        if getattr(request, 'csrf_processing_done', False):
            print("if getattr(request, 'csrf_processing_done', False): === True")
            return None
    ...
    def process_response(self, request, response):
        ...
        if not getattr(request, 'csrf_cookie_needs_reset', False):
            print("if not getattr(request, 'csrf_cookie_needs_reset', False)", not getattr(request, 'csrf_cookie_needs_reset', False))
            if getattr(response, 'csrf_cookie_set', False):
                print("if getattr(response, 'csrf_cookie_set', False):", getattr(response, 'csrf_cookie_set', False))
                return response

        if not request.META.get("CSRF_COOKIE_USED", False):
            print('if not request.META.get("CSRF_COOKIE_USED", False):', not request.META.get("CSRF_COOKIE_USED", False))
            return response
